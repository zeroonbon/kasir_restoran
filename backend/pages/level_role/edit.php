...
$query = "SELECT * FROM level WHERE id_level='$id' LIMIT 1";
$result = mysqli_query($connect, $query);
$level = mysqli_fetch_assoc($result);

if (!$level) {
    die("Level tidak ditemukan!");
}
?>
        ...
        <div class="col-12 px-2 py-2">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4><i class="bi bi-pencil-square me-2"></i> Edit Level</h4>
                    <a href="./index.php" class="btn btn-light text-primary fw-bold rounded-pill px-4 shadow-sm">
                        <i class="bi bi-arrow-left me-1"></i> Kembali
                    </a>
                </div>
                <div class="card-body">
                    <form action="../../actions/level_role/update.php" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="deskripsi_level" name="deskripsi_level" rows="3" required><?= htmlspecialchars($level['deskripsi_level']) ?></textarea>
                        </div>

                        <button type="submit" class="btn btn-primary fw-bold w-100">
                            <i class="bi bi-save me-1"></i> Update Level
                        </button>
                    </form>
                </div>
            </div>
